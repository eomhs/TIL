...
- 라우터들의 집단 네트워크, AS (Autonomous System)
    ...
    - 한 AS 내에는 다수의 라우터가 존재
        ...
        - 이 특별한 라우터를 **AS 경계 라우터(ASBR: Autonomous System Boundary Router)** 라 함
...
- BGP (Border Gateway Protocol)
    ...
    - 엄밀하게는 AS간의 통신이 '가능한' 프로토콜: BGP로 AS 내 라우터 간 통신도 가능
        - eBGP, iBGP
    - 피어링(peering)
...
- AS-PATH 속성
    ...
    - 이런 점에서 경로 벡터 라우팅 프로토콜의 일종이라 하기도 함 
- NEXT-HOP 속성
...
- LOCAL-PREF 속성
    ...
    - LOCAL-PREF 값은 AS 관리 주체가 설정하는 정책의 영향을 많이 받음
- 정책
    - AS 관리 주체에 따라 각기 다른 상이한 정책  사용 가능
    - ec) 특정 AS 우대, 차단, 성능 우선 등등