...
- 물리 계층에는 주소 개념이 없다
    - 단지 호스트와 통신 매체 간의 연결과 통신 매체상의 송수신만 이루어 질 뿐
...
- 허브의 특징이 야기하는 문제, 충돌(collision)
    ...
    - 허브에 호스트가 많이 연결되어 있을수록 충돌 가능성 높아짐
- 콜리전 도메인: 충돌이 발생할 수 있는 영역
    - 허브에 연결된 모든 호스트는 같은 콜리전 도메인
- CSMA/CD
    - 반이중 이더넷 네트워크에서 충돌을 방지하는 대표적인 프로토콜
    - Carrier Sense Multiple Access with Collision Detection
        1) 캐리어 감지: 통신 매체의 현재 사용 여부 검사
        2) 다중 접근: 복수의 호스트가 부득이 동시에 네트워크에 접근할 때
        3) 충돌 검출: 전송 중단, 충돌을 알리는 잼 신호를 보내고 잠시 기다린 뒤 재전송
## 스위치
### 스위치
- 허브의 충돌 문제
...
- 스위치의 MAC 주소 학습 기능
    ...
    1. 플러딩: 허브처럼 모든 포트로 프레임 전송
        - 수신지가 아닌 호스트는 프레임 폐기
        - 수신지인 호스트는 응답 프레임 전송
    2. 포워딩과 필터링
        - 스위치는 수신지가 아닌 포트로는 내보내지 않도록 필터링
        - 수신지인 포트로 프레임을 포워딩
    3. 에이징
        - 만약 MAC 주소 테이블에 등록된 포트에서 일정 시간 동안 프레임을 받지 못하면 해당 항목 삭제
- 스위치의 VALN 기능
    - VLAN: 한 대의 스위치로 가상의 LAN을 만드는 방법
        - 불필요한 트래픽(허브, 스위치의 플러딩)으로 인한 성능 저하 방지
        - 한 대의 스위치로도 여러 개의 스위치가 있는 듯이 호스트의 물리적 위치에 관계없이 나눌 수 있음
    - VLAN은 사실상 다른 LAN: 서로 다른 네트워크로 간주, 브로드캐스트 도메인 달라짐
- VLAN의 종류: 포트 기반 VLAN (port based VLAN)
    - 스위치의 포트가 VLAN을 결정하는 방식
    - 특정 포트에 VLAN을 할당한 뒤, 해당 포트를 호스트에 연결하여 VLAN에 참여
